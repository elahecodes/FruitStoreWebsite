...
import captchaChangeIcon from "../assets/icons/captcha.png";
  ...
  return (
    <section className="w-full">
      <div className="flex justify-start items-center my-8 gap-2">
        <img className="w-5" src={comment} />
        <h3 className="text-[1.4rem] mb-2 font-lalezar">بخش نظرات</h3>
      </div>
      <div className="flex justify-between flex-col-reverse lg:flex-row items-start overflow-scroll no-scrollbar gap-5 lg:gap-0 w-full">
        <div className="lg:w-[65%] w-full max-h-[40rem] no-scrollbar border border-neutral-200 bg-white rounded p-3">
          {UserComments.length === 0 ? (
            <p className="font-lalezar text-neutral-700">
              هنوز نظری ثبت نشده است.
            </p>
          ) : (
            UserComments?.map((item) => (
              <div key={item.id} className="p-2 rounded-lg bg-neutral-100 mb-6">
        ...
        >
          ...
          <div className="w-full flex justify-between items-center gap-1.5">
            <input
              maxLength={5}
              className="md:w-full w-80 bg-white outline-none border h-10 px-2 text-sm rounded border-neutral-200"
              type="text"
              placeholder="کد را وارد کنید"
            />
            <div className="flex justify-center items-center gap-3">
              <img
                className="w-9 bg-white border cursor-pointer border-neutral-200 rounded p-1"
                src={captchaChangeIcon}
                alt=""
              />
              <div className="h-9 md:w-32 w-52 bg-neutral-200 flex justify-center items-center text-lg font-bold">
                354875
              </div>
            </div>
          </div>
